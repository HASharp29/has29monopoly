...
function gameScores(req, res, next) {
    db.many("SELECT Player.name, PlayerGame.score, Game.ID FROM Player, PlayerGame, Game WHERE Player.ID= PlayerGame.playerID and PlayerGame.gameID = Game.ID AND Game.ID = ${id}", req.params)
        .then(data => {
            returnDataOr404(res, data);
        })
        .catch(err => {
